...
Photographs establish identifiers, not the deployed Jira version, metadata permissions, response schemas, historical completeness, plugin semantics or option meanings. No live deployment validation was performed and no private Jira data was used as a test fixture. Points, sprint membership, application and feature IDs are now configured as `photo_confirmed` in the defaults and example, with strict runtime shape checks. Other optional entries remain `pending`. Workflow, history and population validation are separate deployment checks.
...
3. Set `SPRINT_VIEWER_FIELD_MAPPING_FILE` to that JSON path. The three existing `JIRA_*_FIELD` variables remain backward compatible; configured (`validated` or `photo_confirmed`) instance registry fields take precedence. Mapping/configuration and legacy input changes alter the v2 query version. Existing reports retain their frozen configuration.
...
The first implementation recorded the photo IDs but returned an empty registry without a configured file. Its UI also selected the first project/board/sprint, automatically analyzed selection changes, re-rendered unchanged polling results, and discarded usable collected/core and ScriptRunner data when historical prerequisites were missing. A failed metric query could leave dependent history/metrics components missing forever and the grant worker retrying them. Initial tests used configured synthetic histories and did not cover this default deployment path.

The correction supplies the four relevant photo IDs by default, provides an explicit Project → Board → Closed sprint → Analyze flow, removes arrow navigation, increases body/controls to 16px and secondary text to 14px, and stops or backs off polling with a two-minute cap. Terminal dependency failures now propagate into reported status and access-grant scheduling. Authorized original-viewer query totals and collected issue values are retained with separate source/time labels; they do not manufacture baseline/closing history. Added regression tests cover default field configuration, explicit selection, unchanged polling/dropdown preservation, timeout, failed dependencies, and history-unavailable/metric-denied reports.

Correction verification: 118 tests passed (three existing migration deprecation warnings). Focused polling and failed-grant tests also passed after the final polling fingerprint adjustment. Chromium checks cover 1440×1000 desktop and 390×844 mobile, explicit Analyze, dropdown preservation and bounded requests. No live Jira payload reconciliation or deployment configuration changes were performed.
